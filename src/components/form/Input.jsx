...
const Input = React.forwardRef(({ name, label, placeholder, type, register, errors, required, alignLeft }, ref) => (
	<InputDiv alignLeft={alignLeft}>
		<label>{label}</label>
		<input name={name} type={type} ref={ref({ required })} placeholder={placeholder || label} />
